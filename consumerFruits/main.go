...
func main() {
	...
	log.Printf("Binding queue %s to exchange %s with routing key blue", q.Name, "food")
	err = ch.QueueBind(
		q.Name,  // queue name
		"fruit", // routing key
		"food",  // exchange
	...
	tracer := otel.Tracer("consumerFruits")
	var span trace.Span
	...
	go func() {
		for d := range msgs {
			...
			ctx, span = tracer.Start(ctx, "Fruits", trace.WithAttributes(
				label.String("fruit", "yummmm")))
			span.AddEvent("Fruity Fruit", trace.WithAttributes(label.String("message", string(d.Body))))
